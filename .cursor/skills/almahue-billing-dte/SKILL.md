...
- NC/ND: `CodRef` solo 1/2/3 (anula / corrige texto / corrige montos). Razón social emisor máx. 100; el partner no trunca.
...
El intermediario es multi-ERP: no hardcodear sociedades, RUT ni resoluciones. Eso vive en el ERP (Empresa / Admin). En el gateway solo credenciales de **conexión** al partner (`GOSOCKET_API_*`, URLs) y el registry `erpId` → partner.
